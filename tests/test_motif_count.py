...
obs = [(f"s{i}", s) for i, s in enumerate(obs) if s]
...
ctl = [(f"s{i}", s) for i, s in enumerate(ctl) if s]
...
class TestMotifCount(TestCase):
    ...
    def test_profile_to_counts(self):
        ...
        exp = {"TAGT": 1, "TATT": 1, "CTTA": 1, "TACA": 2, "CTCT": 1}
        assert result == exp
    ...
    def test_counts_table(self):
        ...
        assert r.distinct_values("mut") == set("MR")
        # because the motifs are unique in ctl and obs
        # total number should double
        assert r.shape[0] == 2 * (len(obs_count) + len(ctl_count))
    ...
    def test_reduced_one(self):
        ...
        for m, counts in list(r.items()):
            counts = {k[0]: c for k, c in list(counts.items())}
            exp[m] = counts
        assert got == exp
    ...
    def test_combined_counts(self):
        ...
        for b1, b2 in missing:
            sub = combined.filtered(f"base1=='{b1}' and base2=='{b2}'")
            assert sub.summed("count") == 0
