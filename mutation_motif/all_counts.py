...
def check_found_filenames(filenames):
    ...
    if total != 12 or set(found) != set(_directions):
        print("ERROR: counts_pattern did not identify 12 files -- %s" %
              filenames)
        print("Note that each file must contain a single direction pattern"
              ", e.g. CtoT, AtoG")
        exit(-1)


@click.command()
@click.option('-c', '--counts_pattern',
              help='glob pattern uniquely identifying all 12 mutation counts '
              'files.')
@click.option('-o', '--output_path',
              help='Path to write combined_counts data.')
...
@click.option('-p', '--split_dir', default=None,
              help='path to write individual direction strand symmetric '
              'tables.')
